...
- `kyvernoPolicies.enabled` (default `true`) in both charts. Set it to `false` on a cluster that has no Kyverno, where these objects fail the install on an unknown API group: it renders none of the connectivity chart's `kyverno.io` objects, meaning the two kagent declarative-agent ClusterPolicies, the kagent seccomp PolicyException and the agent-sandbox pod-security ClusterPolicy. No installation needs to act. A cluster that enforces restricted PSS through PSA labels instead of Kyverno must set `agentSandbox.enabled: false` as well, since the agent-sandbox controller Deployment then gets its securityContext from nowhere.
...
- `components.<name>.syncWave`, an explicit Argo sync wave for one component. The wave derived from `dependsOn` is binary (1 with dependencies, 0 without), so it cannot order a component behind a dependency that has dependencies of its own. Flux is unaffected, it orders on `dependsOn` directly.
...
- The `agent-sandbox` component is ordered behind the `agent-platform-connectivity` release, through `dependsOn` on the flux engine and `syncWave: "2"` on the argo engine, so the Kyverno mutate policy that carries the controller Deployment's securityContext is loaded before the Deployment applies. The agent-sandbox release no longer fails its first install on a restricted-PSS cluster and then converges on a retry. `dependsOn` waits for the connectivity release to go Ready, which means the ClusterPolicy applied rather than that the Kyverno webhook already serves it, so the release retries still cover a short window.

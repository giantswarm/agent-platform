...
- `components.<name>.versionFromChart`: pin a component to the meta-package's own chart version rather than a range. Set on `agent-platform-connectivity`, the one component released from this repo.
...
- The meta-package pins `agent-platform-connectivity` to its own chart version instead of resolving it from the `>=1.0.0` range. Both charts ship from this repo off the same git tag, but each resolved its own range on its own schedule, so a meta-package on 2.5.12 could pair with connectivity 2.6.0. The meta-package forwards its whole values tree, so that pairing sent the 2.5.12 default `agentic-platform-mcps: {}` into a schema that no longer declared it and the connectivity release failed with `Additional property agentic-platform-mcps is not allowed`. A `components.agent-platform-connectivity.versionRange` in your values still overrides the pin.

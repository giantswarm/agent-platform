...
- The agentgateway controller image is no longer pinned to `v1.2.1`. It now follows the vendored subchart's appVersion (`v1.4.1`), matching the data-plane proxy tag. Gateway API `1.6` (shipped by `gateway-api-bundle` `1.19.0`) serves `TCPRoute` at `v1` only, and `v1.2.1` watches `v1alpha2`: its `TCPRoutes` informer never syncs, `/readyz` never answers, and the install fails the startup probe.

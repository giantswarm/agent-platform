...
- The `agentgateway` component tracks the `2.x` chart line, which flattened the upstream keys onto the chart root. The forwarded values block is no longer nested under an `agentgateway` key, and `agentgateway.enabled` is dropped before forwarding (`omitKeys`) because the flattened chart's schema rejects it. **The top-level `agentgateway:` values block is unchanged**, so no installation has to move an override. `examples/customer-bom.yaml` pins `2.0.0`.

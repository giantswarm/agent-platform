...
- `policyEngine` in the connectivity chart, a toggle for the admission policy engine the target cluster runs. `policyEngine.type: kyverno` (the default) renders the chart's `kyverno.io` objects exactly as before, so no installation needs to act. `policyEngine.type: none` renders none of them (the two kagent declarative-agent ClusterPolicies, the kagent seccomp PolicyException, and the agent-sandbox pod-security ClusterPolicy), for a cluster that has no Kyverno and where those objects fail the install on an unknown API group. The cluster-owned names the PolicyException targets are values now too: `policyEngine.kyverno.policyExceptionNamespace` (`policy-exceptions`), `policyEngine.kyverno.seccompPolicyName` (`restrict-seccomp-strict`) and `policyEngine.kyverno.seccompRuleNames`. They default to the Giant Swarm cluster policy names; a cluster that names its policies differently must override them, or the exception matches nothing.

### Changed

- The agent-sandbox pod-security ClusterPolicy no longer carries `helm.sh/resource-policy: keep`. Helm prunes it now whenever it stops rendering (`policyEngine.type: none`, `agentSandbox.podSecurity.enabled: false`, or the planned v0.5.0 removal of the template), instead of leaving an orphan ClusterPolicy that has to be deleted by hand.
- The `agent-sandbox` component now `dependsOn` the `agent-platform-connectivity` release, so the Kyverno mutate policy that carries the controller Deployment's securityContext is loaded before the Deployment applies. The agent-sandbox HelmRelease no longer fails its first install on a restricted-PSS cluster and then converges on a retry.

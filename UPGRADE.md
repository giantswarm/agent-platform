...
## \<current\> → \<next\> (`VerticalPodAutoscaler`s for the agentgateway data plane and controller behind one `auto` knob; the controller's budget and spread; agentgateway chart 2.1.2)

`gateway.parameters` gains `verticalPodAutoscaler` (`enabled: auto`, `updateMode: Auto`, `minAllowed` 50m/64Mi, `maxAllowed` 2/2Gi): the connectivity chart renders a `VerticalPodAutoscaler` named after `gateway.name` on the data-plane `Deployment` when `autoscaling.k8s.io/v1` is served (`templates/agentgateway/verticalpodautoscaler.yaml`) — resolved once by the meta chart like the other cluster-shape knobs and forwarded resolved. The forwarded `agentgateway:` block gains `controller.podDisruptionBudget: {maxUnavailable: 1}`, a root `topologySpreadConstraints` entry on `kubernetes.io/hostname` selecting the controller pods (`agentgateway: agentgateway`) and `controller.verticalPodAutoscaler` (the same shape, 50m/128Mi–2/2Gi), which the meta chart empties to `{}` where the knob resolves false. `components.agentgateway.versionRange` is `>=2.1.2 <3.0.0` (was `2.x`); `examples/customer-bom.yaml` pins `2.1.2`.

### Merge-order dependency

- **This release needs the agentgateway chart 2.1.2 published** — giantswarm/agentgateway#51, which makes the packaging chart's schema accept `controller.podDisruptionBudget`, `controller.verticalPodAutoscaler`, `topologySpreadConstraints` and the other controller keys; 2.1.1 declares them as empty objects and refuses every key inside (`additional properties 'maxUnavailable' not allowed`). The range floor keeps the two from meeting on a cluster: until 2.1.2 exists in `oci://gsoci.azurecr.io/charts/giantswarm/agentgateway`, the agentgateway `OCIRepository` resolves nothing (its Ready condition reports that no tag satisfies `>=2.1.2 <3.0.0`), the agentgateway `HelmRelease` reports its source not ready, and **every release that depends on it holds**: the connectivity release and agent-platform-mcps `dependsOn` agentgateway directly, and kagent, the managers, the portal and the rest depend on connectivity, so no new generation of the platform applies on that installation until 2.1.2 exists (every running release keeps running at its current revision). The moment 2.1.2 is published Flux resolves it, upgrades the controller with the new values in one go and the held releases proceed. Merge and release giantswarm/agentgateway#51 **before** this; do not lower the floor to make the release resolve — 2.1.1 would then refuse the values on every installation with the component on.
- `make verify-components-charts` is red until 2.1.2 is published: it renders the forwarded `agentgateway` block against the newest published chart (2.1.1), which rejects the keys. That is the intended signal, not a regression to paper over.

### Operator action

- **None** for the default shape on a management cluster (the VPA API served). Once the `OCIRepository` resolves 2.1.2 the controller rolls once — the chart upgrade, the `PodDisruptionBudget`, the spread constraint and its `VerticalPodAutoscaler` arrive together (the spread is one Deployment revision); the VPA then re-sizes the controller pods by eviction once it has a recommendation, one pod at a time behind the budget, requests only and never above the chart's fixed limits (500m/512Mi). The data-plane `VerticalPodAutoscaler` is a new object and changes no pod until the recommender's first eviction, one pod at a time behind the data-plane budget. Both VPAs run in `Auto` mode, like the VPAs the fleet already runs.
- **Clusters without the VPA CRD** (kind labs, plain clusters): nothing autoscaling-related renders — the knob resolves `false`, the connectivity chart renders no data-plane VPA and the meta chart forwards `agentgateway.controller.verticalPodAutoscaler: {}` (the packaging chart renders no VPA from an empty object). Installing the VPA and reconciling turns both on. The controller's budget and spread render on every cluster.
- **Opt-outs**: `gateway.parameters.verticalPodAutoscaler.enabled: false` renders neither VPA (with the API served too); `agentgateway.controller.verticalPodAutoscaler: null` removes the controller's alone; `gateway.parameters.verticalPodAutoscaler.updateMode: Off` keeps the data-plane recommendations without applying them (`Initial` applies them at pod creation only), and `minAllowed` / `maxAllowed` bound the size. The controller's bounds are the keys of `agentgateway.controller.verticalPodAutoscaler`.
- A single-node lab with the VPA installed: an `Auto` eviction goes through the budget one pod at a time and the replacement schedules on the same node (unlike a drain, nothing is cordoned), so VPA evictions complete there; a drain of that node still waits on the budget (#373).
- A BOM that pins `components.agentgateway.versionRange` below `2.1.2` next to these values fails the agentgateway release on the chart's schema; pin `2.1.2` or above.
...
- **An RPC the kagent line adds** (a re-pin with a new method on one of the five services) is reachable through the route only once it is listed in `kagent.controllerRoute.grpc.services` (map service → methods; the chart's default is the line's `proto/` at its pin). An unlisted RPC fails loudly — the MCP catch-all route answers `406` — never silently. The list exists because the agentgateway controller (v1.5.x) translates a service-only match into an exact path no request has; it goes back to service-only matches once the agentgateway line carries the fix.
...
- **`agent-manager.muster.url`: leave it unset.** The meta chart derives it from `muster.fullnameOverride` and `muster.service.port`; a set value that differs fails the render naming both. On a default install the derived URL equals the agent chart's own default (`http://muster.agent-platform.svc.cluster.local:8090/mcp`), so nothing changes in the composed agents. The portal reads `agentPlatform.musterMcpUrl` from the app-config the connectivity chart renders (giantswarm/backstage#2296).

...
- **Skipping 4.7.19** (from 4.7.18 or earlier straight to 4.8.0): the connectivity upgrade deletes the Harness (no `keep` in the previous manifest) and the kagent upgrade recreates it one reconcile later. Templates are re-admitted, snapshots stay in the store; running turns in that window fail and are retried by the caller. Upgrade through 4.7.19 to avoid the gap.

...
- **Images verified from another registry** (`imageVerification.images` names it): add its hosts to `kyvernoEgress.hosts`, as `matchName` entries or a `matchPattern` (`*` matches one label of a name).

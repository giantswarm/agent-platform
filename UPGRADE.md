...
- **A backend registered at runtime and an enforcing network policy**: the connectivity chart's egress policy for model-manager is rendered from the static inputs; a backend registered at runtime on the cluster network (an in-cluster Ollama on `:11434`) is not opened by it under the cilium flavour yet. `modelManager.networkPolicy.egress` opens 443 to the listed CIDRs today; the documented input for registered backends is giantswarm/agent-platform#478.

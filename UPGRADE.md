...
- **An installation still pinned to a 1.x gateway** (`components.klaus-gateway.versionRange` below `2.0.0`): act before you take this release. A 1.x chart reads the keys, and without them it falls back to its own defaults — `lifecycle.driver: operator` with no `operatorMCPURL`, where this chart forwarded `static`. Set `klausGateway.lifecycle.driver: static` in your own values, or move the pin to the 2.x line, which ignores the key entirely. The other five defaults match what this chart forwarded, so they change nothing.

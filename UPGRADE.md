...
- `make verify-components-charts` is red until 2.1.2 is published: on this line it pulls the chart at the range and no published version satisfies `>=2.1.2 <3.0.0` (on main it renders the forwarded block against 2.1.1, which rejects the keys). That is the intended signal, not a regression to paper over.

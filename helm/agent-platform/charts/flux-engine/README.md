...
| instance.distribution.registry | string | `"gsoci.azurecr.io/giantswarm/fluxcd"` | Registry the Flux controller images are pulled from: retagger's copy of ghcr.io/fluxcd under its upstream path (source-controller and helm-controller of every 2.9+ release, giantswarm/retagger#1229); the operator composes <registry>/<controller>:<tag>. Another mirror goes here. |

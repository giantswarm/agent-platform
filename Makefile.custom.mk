...
verify-disruption: ## Assert the voluntary-disruption guards (giantswarm/agent-platform#431): karpenter.sh/do-not-disrupt on the agentgateway data plane (AgentgatewayParameters overlay), muster, kagent-controller, klaus-gateway and muster-valkey (their charts' podAnnotations); PodDisruptionBudgets from the muster, kagent and klaus-gateway charts' knobs and the connectivity chart's own for agent-manager and muster-valkey (#439); every knob off = nothing; the budgets' guards. And the placement of the stateful singletons (#439): scheduling.singletons.nodeSelector / tolerations reach muster, muster-valkey, the kagent controller and klaus-gateway as their charts' knobs (a component's own keys win), never the connectivity release; empty = nothing forwarded.
	...
	@grep -q '^      template:$$' /tmp/vd-params.out || { echo "FAIL: AgentgatewayParameters deployment overlay lost its pod template"; exit 1; }
	@grep -A2 '^        metadata:$$' /tmp/vd-params.out | grep -q 'karpenter.sh/do-not-disrupt: "true"' || { echo "FAIL: the data-plane pod template carries no karpenter.sh/do-not-disrupt annotation"; cat /tmp/vd-params.out; exit 1; }
	@echo "ok: data-plane pods annotated through deployment.spec.template.metadata"
	@awk '/^kind: PodDisruptionBudget/,/^---/' /tmp/vd-on.out >/tmp/vd-pdb.out
...
# placeholders; this target writes an overlay from the environment into a
# temporary file that is removed on exit, and never prints a value.
SCENARIO ?= smoke
CLUSTER_TYPE ?= kind
...
# the scenario's own list, so it must be complete: the kind smoke needs
...
# E2E_DOMAIN, E2E_ISSUER_URL, E2E_CLIENT_ID, E2E_CLIENT_SECRET_ENV (the NAME of
# the variable holding the secret, never the secret itself on the command line).
E2E_DOMAIN ?=
E2E_ISSUER_URL ?=
E2E_CLIENT_ID ?=
E2E_IDP_SECRET_NAME ?=
...
e2e: ## Run one ATS scenario against any cluster (KUBECONFIG=… [SCENARIO=smoke|functional] [CLUSTER_TYPE=kind|eks] [VALUES=a.yaml:b.yaml]). Packages the chart first; builds a values overlay from the environment so no secret or domain is committed.
	...
	@test -r "$(KUBECONFIG)" || { echo "FAIL: cannot read KUBECONFIG=$(KUBECONFIG)"; exit 1; }
	@mkdir -p $(E2E_DIST)
	@helm package $(CHART_DIR) --version $(E2E_VERSION) -d $(E2E_DIST) >/dev/null
	@archive=$(E2E_DIST)/agent-platform-$(E2E_VERSION).tgz; \
	test -f "$$archive" || { echo "FAIL: helm package produced no $$archive"; exit 1; }; \
	overlay=""; \
	if [ -n "$(E2E_DOMAIN)$(E2E_ISSUER_URL)$(E2E_CLIENT_ID)$(E2E_IDP_SECRET_NAME)" ]; then \
		overlay=$$(mktemp -t agent-platform-e2e-XXXXXX.yaml); \
		trap 'rm -f "$$overlay"' EXIT INT TERM; \
		{ echo "# Generated by make e2e from the environment. Not committed."; \
		  echo "global:"; \
		  [ -n "$(E2E_DOMAIN)" ] && echo "  domain: $(E2E_DOMAIN)"; \
		  echo "  identity:"; \
		  [ -n "$(E2E_ISSUER_URL)" ] && echo "    issuerUrl: $(E2E_ISSUER_URL)"; \
		  [ -n "$(E2E_CLIENT_ID)" ] && echo "    clientId: $(E2E_CLIENT_ID)"; \
		  [ -n "$(E2E_IDP_SECRET_NAME)" ] && echo "    existingSecret: $(E2E_IDP_SECRET_NAME)"; \
		  true; } > "$$overlay"; \
	...
	ATS_CHART_PATH=$(abspath $(E2E_DIST))/agent-platform-$(E2E_VERSION).tgz \
	ATS_CHART_VERSION=$(E2E_VERSION) \
	ATS_CLUSTER_TYPE=$(CLUSTER_TYPE) \
	$${overlay:+ATS_OVERLAY_VALUES=$$overlay} \
	$(if $(VALUES),ATS_VALUES=$(VALUES),) \

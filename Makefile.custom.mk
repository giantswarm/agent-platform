...
VM_BASE := --set ingress.parentRefs[0].name=x
...
# The 3.x line: main is the kagent API v2 line (4.x) and renders differently by
# design, so the golden is this branch's own tip. A pull request against
# release-v3.x compares its render with what the fleet runs.
...
# PICK selects one object of a render by kind and name (tests/pick-doc.py; an
# awk range picks the first object of a kind, wrong as soon as two render).
...
verify-components-charts: ## Pull the component charts the meta chart composes values for — the seven extras and agentgateway (a closed schema: a forwarded key it does not declare fails the release; agentgateway's controller budget, spread and VPA need 2.1.2, so this is RED until giantswarm/agentgateway#51 is released) — at the range's resolution and at the BOM pin, resolved the way Flux does, and render each with the values the meta chart forwards to it. Network: gsoci.azurecr.io, ghcr.io.
	@echo "====> $@ ($(CHART_DIR))"

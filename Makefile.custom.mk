...
verify-meta: ## Assert the app-of-apps meta-package render (pure renderer with the engine off, ranges as values, Flux the only engine, pinned BOM).
	...
	@echo "--> mirrored defaults: the meta chart's copy of every networkPolicy fqdns / cidrs list and port, of every leaf of the modelServing namespace, cache, policies, prepull, imageVerification, runtime and additionalRuntimes blocks and of the clusterManager prewarmPriorityClass block, equals the connectivity chart's default — the forwarded copy shadows the child's (#522, #525, #537, #539, #545, #550, #552, #565)"
...
verify-serving-slice: ## Assert the serving slice (giantswarm/agent-platform#326): examples/serving-slice.yaml renders exactly the five kserve releases and connectivity (no muster, dicebear, valkey, kagent, Backstage, agent-manager, model-manager, agentgateway; the engine off) — kserve-runtime-configs after kserve-llmisvc-crd into the kserve namespace with the well-known configs on, the runtimes off, the llm-d-fast/ prefix as imageRegistry — the prefix the pre-pull's llm-d-cuda reference carries (#568) — and its block held back from connectivity; KServe's ingress-gateway value derived onto kserve-resources (a differing copy fails, an equal one is a no-op, none with the Gateway off); runtimeClassName nvidia forwarded; the target knob adds agentgateway and stamps every kubeConfig. The connectivity chart with the forwarded values: the models Gateway on models.<global.domain> with the wildcard Secret and the external-dns hostname, ONE Strict AgentgatewayPolicy on the Gateway (audience dex-k8s-authenticator, inheritance Override, the issuer), the JWKS backend at the issuer's host on 443 with TLS, the discovery ConfigMap's gateway entry; a Certificate only with tls.issuerRef.name; nothing with modelsGateway.enabled false (the default: the Gateway is the slice's, the profile turns it on); the guards name their key. The two 24 GB presets: schema keys, no image, tools on with a parser, one GPU, <= 24 GiB, requests within what a g6.xlarge leaves a predictor after the kubelet's reservations and the daemonsets (#502), the description naming the instance. Every shipped preset's arguments survive both runtimes' entrypoints: the llm-d template's exact eval "… $@" (#532) and the classic kserve-vllm ClusterServingRuntime's RENDERED command — /bin/sh -c 'eval "exec vllm serve $@"' -- ahead of the base arguments (#549) —, each run over the preset's args with argv dumped in place of vLLM, yield one word per argument and every JSON value parses; the classic runtime's base arguments (--served-model-name {{.Name}} for KServe to expand, an installation's extra ones) reach vLLM one word each; a values preset whose argument carries whitespace, a quote or a shell metacharacter outside single quotes fails the render naming the guard. The cache claim (#483): no PersistentVolumeClaim object — a post-install,post-upgrade hook Job server-side applies hf-cache (keep, RWO, 100Gi on the chart's gp3 class at 500 MiB/s / 3000 IOPS, named with a digest of provisioner and parameters — a parameter change renders a new class, #570; the class incl. "-", size, volumeName and access-mode knobs) as <release>-hooks with get/create/patch on claims — its script, run against a stub kubectl, keeps an existing claim's class and size (grows, never shrinks; a Pending claim on a class the cluster lacks fails naming the way out); cache.enabled false or an existing claim render neither hook nor identity; the serving namespace carries helm.sh/resource-policy: keep with the cache on and off (#565: a namespace kept only with the cache on took a claim an earlier slice left there down with it), not with namespace.keep false. The live half (a served LLMInferenceService answers 200 with a person's id_token and 401 without; no bearer in the model server's log) runs on a GPU cluster: README "The serving slice and the models Gateway". HELM selects the binary.
	@echo "====> $@ ($(CHART_DIR), $(CONNECTIVITY_DIR))"
...
verify-wiring: ## Assert the standalone's ported wiring: toggles off = no object; on = the Backstage app-config (one-value identity), route and config-reload hook, the mcp-kubernetes MCPServer (OAuth, forwarded token, kube audience), the model serving objects on the kserve components and the guard without them, the KServe controller policies; the meta chart forwards the blocks, omits the wiring keys and renders no release for the switch.
	...
	@echo "--> modelServing + kserve components on (fleet shape, kagent on): runtime, namespace, discovery ConfigMap, presets, chat template, the cache claim's hook Job (no PVC object: #483) and its identity, the two Kyverno policies, the cilium policies incl. the agent egress, the kserve controller policy"
	...
	@for pattern in 'kind: ClusterServingRuntime' '^  name: kserve-vllm$$' 'image: "docker.io/vllm/vllm-openai:' '^  name: agent-platform-model-serving$$' '^  name: t-model-serving-cache$$' 'helm.sh/hook: post-install,post-upgrade' '"kind":"PersistentVolumeClaim"' '"name":"hf-cache","namespace":"model-serving"' 'kubectl apply --server-side' 'resources: \["persistentvolumeclaims"\]' 'agent-platform-serving-preset-qwen3-8-27b' 'agent-platform-serving-preset-qwen3-14b' 'name: agent-platform-chat-template-qwen3-8-27b' '^  name: model-serving$$' 'kind: Namespace' 'name: agent-platform-connectivity-model-serving-pods' 'name: agent-platform-connectivity-model-serving-deployments' 'redirectPolicy: true' 'name: agent-platform-connectivity-model-serving-predictor$$' 'name: agent-platform-connectivity-model-serving-download$$' 'name: agent-platform-connectivity-kagent-agents-to-model-serving' 'matchName: huggingface.co' 'matchPattern: "\*"' '- remote-node' 'name: agent-platform-connectivity-kserve-controller' 'name: agent-platform-connectivity-llmisvc-controller' 'control-plane: kserve-controller-manager' 'flavor: cilium' 'preset-source: "shipped"'; do \

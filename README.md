...
Measured (each test logs `TIMING <phase>`; CI = the 4-vCPU `large` machine with cold pulls; the per-phase table with the Substrate sizing is in [tests/ats/README.md](tests/ats/README.md#the-cluster-kind-with-agent-substrate)): **smoke** — `helm install --wait` TBD_INSTALL s, Substrate ready TBD_SUBSTRATE s after the install returned, the declarative `AgentTemplate` Ready on the Harness TBD_DECL s, agent-manager `create_agent` → HelmRelease Ready → `AgentTemplate` Ready + `RemoteMCPServer` Accepted TBD_MANAGED s, the fixpoint wait fills the rest of two 1-minute intervals after the adoption, `helm upgrade` refused in ~1 s, `helm uninstall --wait` TBD_UNINSTALL s (the kagent namespace is kept). **functional** — the chart through the cluster's Flux TBD_FLUX_PLATFORM s, the agent through that Flux TBD_FLUX_AGENT s. The whole `execute-chart-tests` job: **TBD_JOB** (kind create ~40 s, the ATS image pull and `uv sync` ~1 min, the two pytest runs the rest). The long pole is the two platform installs — every component image pulled once; Substrate's control plane, the worker image and the Go ADK image the Harness boots are the additions of kagent API v2.

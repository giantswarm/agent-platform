...
A cluster that runs its own Flux — every Giant Swarm management cluster — sets `components.flux.enabled: false` and installs the chart through that Flux. Nothing of the engine reaches the cluster: no CRD, no operator, no `FluxInstance`, no hook, no tenant identity, and the platform `HelmRelease`s carry no `serviceAccountName` unless `gitops.serviceAccountName` says so. The cluster also owns its namespaces: with kagent on, create the `kagent` namespace (`kagent.namespaceOverride`) before the chart installs — the Giant Swarm management-cluster bases do — because the kagent release puts its objects there and the connectivity release that renders the `Namespace` depends on kagent for its CRDs (with the engine the chart creates it itself, see [The engine](#the-engine)). Self-management is off with the engine (`gitops.self.enabled: auto`) — that Flux holds the chart's `HelmRelease`, as below. The render is the pure app-of-apps render, byte for byte what it was before the engine existed.
...
Measured (each test logs `TIMING <phase>`; local = a 24-core kind node with warm image caches, CI = the 2-vCPU `medium` machine with cold pulls): **smoke** — prerequisites 4 s, registry + push 3 s, `helm install --wait` 97 s local / CI_INSTALL, adoption 5–11 s after the install returned, muster healthy 0 s, the auth round trips 1 s, the declarative Agent Ready 5–10 s, agent-manager `create_agent` → HelmRelease Ready → Agent Ready 6 s local / CI_CREATE, the fixpoint wait fills the rest of two 1-minute intervals after the adoption, `helm upgrade` refused in 1 s, `helm uninstall --wait` 65–69 s local / CI_UNINSTALL (the long pole of the teardown is the kagent namespace's termination — the connectivity release owns the `Namespace`, its pods and PVC go with it; 12–16 s without kagent); the smoke's pytest run 5 min local / CI_SMOKE. **functional** — `flux install` 19 s, the chart through the cluster's Flux 112 s local / CI_HR, the agent through that Flux 36 s, the guard firing 16 s after the flip, the way back 106 s; the pytest run 5 min local / CI_FUNCTIONAL. The whole `execute-chart-tests` job: CI_TOTAL (kind create ~40 s, the ATS image and `uv sync` ~1 min on top). The long pole of the job is the two platform installs on two cores — every component image pulled once, kagent's controller and bundled Postgres the slowest to become Ready.

...
On the dev line (poc/kagent-main) the smoke values leave kagent and
agent-manager off (values-kagent.yaml says why: kagent main cannot run on the
ATS kind cluster), so step 6 is skipped with that reason and the kagent-only
assertions (the kagent namespace, the orphaned agents) are gated on KAGENT_ON /
AGENT_MANAGER_ON, both read from the values.
"""
...
# only when the values turn them on — not on the dev line, see conftest).

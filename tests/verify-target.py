...
def hold_retired_servicemonitor(here: str, there: str) -> tuple:
    ...
    return h, strip(there)
# giantswarm/giantswarm#37705: the kagent and Substrate lines (1.0 or 1.1) and
# the Generic agent chart (<1.5 or >=1.5) may differ between this tree and
# GOLDEN_REF: #665 moved them to 1.1, its revert back to 1.0. Written on BOTH
# sides so the ranges compare equal; dropped once both sides agree.
...
# platform Harness's compaction (kagent.harness.compaction); a 1.0 side has
# neither. Both blocks are blanked on BOTH sides; drop once both sides agree.
...
def hold_credential_provider_exception(here: str, there: str) -> tuple:
    ...
    def strip(render: str) -> str:
        # Trailing blank lines are trimmed on both sides: the exception is the
        # last document of its template, so whichever side renders it leaves the
        # blank line before it on the document it followed.
